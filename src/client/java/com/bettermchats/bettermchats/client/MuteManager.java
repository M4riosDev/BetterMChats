...
public class MuteManager {
    ...
    public static void init() {
        try {
            configDir = new File(MinecraftClient.getInstance().runDirectory, "config/bettermchats");
            if (!configDir.exists()) {
                configDir.mkdirs();
            }
            loadMutedChannels();
        } catch (Exception e) {
            BetterMChats.LOGGER.error("[BetterMChats] Failed to initialize MuteManager", e);
        }
    }
    ...
    private static void loadMutedChannels() {
        try {
            ...
            BetterMChats.LOGGER.error("[BetterMChats] Failed to load muted channels", e);
        }
    }
    ...
    private static void saveMutedChannels() {
        try {
            ...
            BetterMChats.LOGGER.error("[BetterMChats] Failed to save muted channels", e);
        }
    }
