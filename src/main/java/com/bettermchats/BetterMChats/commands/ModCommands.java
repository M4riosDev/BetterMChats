...
public class ModCommands {
                        ...
                        .executes(ctx -> {
                            ...
                            if (action.length() > MeAboveHeadPacket.MAX_TEXT_LENGTH) {
                                ctx.getSource().sendFeedback(
                                    new StringTextComponent("[BetterMChats] Action message too long (max "
                            ...
                            if (!RATE_LIMITER.canMessage(player.getUniqueID(), "me", ME_COOLDOWN_MS)) {
                                ...
                                ctx.getSource().sendFeedback(
                                    new StringTextComponent("[BetterMChats] You must wait " + seconds
                        ...
                        .then(Commands.literal("chats")
                            .then(Commands.literal("all")
                                .executes(ctx -> {
                                    ...
                                    ctx.getSource().sendFeedback(
                                        new StringTextComponent("[BetterMChats] cleared chats for all players (" + cleared + ")."),
                                        false);
                                    return 1;
                                }))
                ...
                .then(Commands.argument("message", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            if (!RATE_LIMITER.canMessage(player.getUniqueID(), cmd, cooldown)) {
                                ...
                                int seconds = (int) Math.ceil(remaining / 1000.0);
                                ctx.getSource().sendFeedback(
                                    new StringTextComponent("[BetterMChats] You must wait " + seconds + " second(s) before sending another /" + cmd + " message."),
                                    false);
                                return 0;
                            }
    ...
    private static int runClearByRole(CommandSource source, String role) {
        ...
        if (cleared == 0) {
            source.sendErrorMessage(new StringTextComponent("[BetterMChats] no players found with role '" + role + "'."));
            return 0;
        }

        source.sendFeedback(
                new StringTextComponent("[BetterMChats] cleared chats for role '" + role + "' (" + cleared + ")."),
                false);
        return 1;
    }
    ...
    private static int runClearByUsername(CommandSource source, String username) {
        ...
        source.sendFeedback(
                new StringTextComponent("[BetterMChats] cleared chats for player '" + username + "' (" + cleared + ")."),
                false);
        return 1;
    }
